...
ADDR_RE = re.compile(r'^\s*/\*\s*([0-9A-Fa-f]+)(?:\s+([0-9A-Fa-f]+))?[^*]*\*/')


def label_address(match):
    """The vaddr out of an ADDR_RE match."""
    return int(match.group(2) or match.group(1), 16)
# The disassembler marks a run of bytes that no symbol claims with this
# comment, on the line before the label it invents for it. The comment is the
# signal rather than the label's name: the prefix depends on whether
# autogenerated names are being taken from the section a symbol sits in, and a
# real unnamed object is spelled the same way as a pad when they are not.
PAD_COMMENT = 'Automatically generated and unreferenced pad'
PAD_NAME_RE = re.compile(r'^[BD]_[0-9A-Fa-f]+$')
...
def symbol_sections():
    ...
    Derived rather than written down: the splat configuration records both the
    section layout and every symbol's address, so the dump a symbol belongs to
    follows from where it sits. A symbol in no dump -- every .rodata constant,
    which travels with the function that uses it instead -- simply does not
    appear.
    """
    where = {}
    for image, rows in disassemble.read_symbol_table().items():
        for name, (address, _type, _size) in rows.items():
            dump = disassemble.dump_of(image, address)
            if dump:
                where[name] = dump
    return where


def manifest_by_section(manifest_path):
    """{dump: {symbol: (object, elf_section)}} for the dumps that get carved."""
    where = symbol_sections()
    out = {}
...
def compute_layout(lines, migrated):
    ...
    padded = set()
    for li, name, _addr in labels:
        for prev in range(li - 1, -1, -1):
            if lines[prev].strip() == '':
                continue
            if PAD_COMMENT in lines[prev]:
                padded.add(name)
            break

    def is_pad(name):
        return name in padded and bool(PAD_NAME_RE.match(name))
