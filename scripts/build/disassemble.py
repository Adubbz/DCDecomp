...
def configure_spimdisasm():
    ...
    def configure(self):
        ...
        # .bss, `RO_` in .rodata. scripts/build/migrate.py recognises padding
        # by that prefix, and without it the holes come out too small.
...
def fix_branches(root):
    ...
    """
    paths = sorted(Path(root).rglob("*.s"))
...
def clear_generated():
    ...
    the old file behind defining the same symbol twice. Only asm/handwritten,
    which splat does not produce, is kept.
    """
    removed = 0
    root = Path("asm")
    if not root.is_dir():
